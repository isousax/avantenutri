  ...
  return useQuery<FetchResponse>({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        const res = await fetch(`${API.CONSULTATION_CREDITS_SUMMARY}?all_by_user=1`, { 
          headers: { Authorization: `Bearer ${token}` } 
        });
        
        if (res.status === 500) {
          console.error('Server error fetching admin credits:', res.status);
          throw new Error('Erro interno do servidor ao buscar créditos. Tente novamente em alguns instantes.');
        }
    ...
    retry: (failureCount, error) => {
      ...
      if (error.message.includes('permissão') || error.message.includes('Token')) {
        return false;
      }
    ...
    mutationFn: async (vars: { userId: string; type: 'avaliacao_completa' | 'reavaliacao'; delta: number; reason?: string }) => {
      ...
      try {
        ...
        if (res.status === 500) {
          ...
          throw new Error('Erro interno do servidor ao ajustar créditos. Tente novamente em alguns instantes.');
        }

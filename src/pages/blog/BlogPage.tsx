...
const BlogPage: React.FC = () => {
  ...
  const categories = [
    ...
    { id: 'saude', name: 'Saúde & Bem-estar', count: blogPosts.filter(p => p.category === 'saude').length },
    { id: 'dietas', name: 'Dietas Específicas', count: blogPosts.filter(p => p.category === 'dietas').length },
    { id: 'gestacao', name: 'Nutrição na Gestação', count: blogPosts.filter(p => p.category === 'gestacao').length },
    { id: 'esporte', name: 'Nutrição Esportiva', count: blogPosts.filter(p => p.category === 'esporte').length },
    { id: 'dicas', name: 'Dicas Práticas', count: blogPosts.filter(p => p.category === 'dicas').length },
    { id: 'ciencia', name: 'Ciência da Nutrição', count: blogPosts.filter(p => p.category === 'ciencia').length }
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-green-25">
      ...
      <section className="bg-green-800 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Blog de Nutrição</h1>
          <p className="text-xl text-green-100 max-w-3xl mx-auto">
            Artigos, dicas e informações baseadas em ciência para ajudar você na jornada 
            de uma alimentação mais saudável e consciente.
          </p>
            ...
            {featuredPost && (
              <section className="mb-12">
                ...
                <Card className="overflow-hidden hover:shadow-xl transition-all duration-300">
                  <Link to={`/blog/${featuredPost.id}`} className="flex flex-col md:flex-row">
                        ...
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-500">
                              {new Date(featuredPost.publishDate).toLocaleDateString('pt-BR')} • {featuredPost.readTime} min de leitura
                            </p>
                          </div>
                        </div>
          ...
          <aside className="lg:w-1/4 space-y-8">
            ...
            <Card className="p-6">
              ...
              <p className="text-gray-600 text-sm mb-4">
                Espaço dedicado à compartilhar conhecimento científico sobre nutrição, 
              ...
              <div className="flex items-center">
                ...
                <div>
                  <p className="font-medium text-gray-900">Dra. Andreina Cawanne</p>
                  <p className="text-sm text-gray-500">Nutricionista CRP-XXXXX</p>
                </div>
              </div>
            </Card>
            ...
            <Card className="p-6">
              ...
              <div className="space-y-2">
                {categories.filter(cat => cat.id !== 'todos').map(category => (
                  ...
                  >
                    <span className={`${
                      selectedCategory === category.id ? 'text-green-600 font-medium' : 'text-gray-600'
                    }`}>
                      {category.name}
                    </span>

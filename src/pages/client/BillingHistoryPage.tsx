...
export default function BillingHistoryPage(){
  ...
  if(!confirm(t('billing.history.downgrade.confirm'))) return;
    setDowngrading(true);
    try {
  const token = await getAccessToken(); if(!token) { push({ type:'error', message: t('auth.error.loginRequired')}); return; }
  ...
  if(!res.ok) { push({ type:'error', message: data.error || t('billing.history.downgrade.error')}); return; }
  ...
  return (
    <div className='max-w-4xl mx-auto p-6'>
      ...
      <div className='flex gap-4 mb-6'>
        ...
        <button disabled={downgrading} onClick={downgrade} className='px-3 py-1.5 rounded bg-rose-600 text-white text-sm disabled:opacity-50'>{t('billing.history.downgrade.button')}</button>
      </div>

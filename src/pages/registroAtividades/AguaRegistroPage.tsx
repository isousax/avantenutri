...
const AguaRegistroPage: React.FC = () => {
  useEffect(() => {
    ...
    if (metaDescription) {
      metaDescription.setAttribute('content', 'Registre e acompanhe seu consumo diário de água com nossa ferramenta de controle de hidratação.');
    }
  }, []);
  ...
  const adicionarCopo = () => {
    if (coposHoje < 20) { // Limite razoável
  ...
  const getMensagemMotivacional = (): MensagemMotivacional => {
    ...
    if (progresso >= 100) return { mensagem: "Parabéns! Meta atingida! 🎉", cor: "text-green-600" };
    if (progresso >= 75) return { mensagem: "Quase lá! Continue assim! 💪", cor: "text-blue-600" };
    if (progresso >= 50) return { mensagem: "Bom trabalho! Metade do caminho! 👍", cor: "text-yellow-600" };
    return { mensagem: "Vamos começar! Cada copo conta! 💧", cor: "text-orange-600" };
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-cyan-50 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-blue-800 mb-2">Controle de Hidratação</h1>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Card className="p-6 shadow-xl border border-blue-100">
            ...
            {/* Contador */}
            <div className="text-center mb-6">
              <div className="text-6xl font-bold text-blue-600 mb-2">{coposHoje}</div>
            ...
            <div className={`text-center p-4 rounded-lg mb-6 ${
              mensagem.cor === 'text-green-600' ? 'bg-green-600' :
              mensagem.cor === 'text-blue-600' ? 'bg-blue-600' :
              mensagem.cor === 'text-yellow-600' ? 'bg-yellow-600' :
              'bg-orange-600'
            } bg-opacity-10`}>
          ...
          <div className="space-y-6">
            ...
            <Card className="p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Histórico de Consumo de Água da Última Semana</h3>
              <div className="space-y-3" role="list" aria-label="Histórico de consumo de água">
                {historico.map((registro, index) => (
                  <div key={index} className="flex justify-between items-center p-3 border border-gray-100 rounded-lg">
                    <div>
                      <p className="font-medium text-gray-900">
                        {new Date(registro.data).toLocaleDateString('pt-BR', { weekday: 'short' })}
                      </p>
                    ...
                    <div className="text-right">
                      <p className={`font-bold ${
                        registro.copos >= registro.meta ? 'text-green-600' : 'text-orange-600'
                      }`}>
                        {registro.copos} copos
                      </p>
                      <p className="text-sm text-gray-500">meta: {registro.meta}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <Card className="p-6 bg-cyan-50 border border-cyan-100">
              <h3 className="text-lg font-semibold text-cyan-800 mb-3">💡 Dicas de Hidratação</h3>

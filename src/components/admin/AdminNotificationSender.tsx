...
const AdminNotificationSender: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    target_users: '',
    target_group: 'active' as 'active' | 'incomplete_questionnaire' | 'recent_signups',
    expires_days: 30
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.target_type === 'specific') {
      ...
      if (userIds.length === 0) {
        push({ type: 'error', message: 'IDs de usuários são obrigatórios para envio específico' });
        return;
      }
  ...
  return (
    <Card className="p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-xl font-bold text-gray-900">Enviar Notificação</h2>
          <p className="text-sm text-gray-600">Envie notificações para usuários ou grupos específicos</p>
        </div>
      </div>
        ...
        {formData.target_type === 'group' && (
          <div>
            ...
            >
              ...
              <option value="incomplete_questionnaire">Usuários sem questionário</option>
              <option value="recent_signups">Novos usuários (últimos 7 dias)</option>
            </select>
          </div>
        )}

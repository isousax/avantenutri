...
const StructuredDietBuilder: React.FC<BuilderProps> = ({
  ...
  function handleAltSearchChange(itemId: string, value: string) {
    setAltSearch((prev) => ({
      ...prev,
      [itemId]: value,
    }));
  ...
  const totalItems = data.meals.reduce(
    (sum, meal) => sum + meal.itens.length,
    0,
  );
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-blue-700">
              {totalItems} alimento{totalItems !== 1 ? "s" : ""} •{" "}
              {data.meals.length} refeiç{data.meals.length !== 1 ? "ões" : "ão"}
            </div>
          </div>
          {data.total && (
            <div className="text-right">
              <div className="text-sm font-bold text-blue-900">
                {data.total.calorias} kcal
              </div>
              <div className="text-xs text-blue-700">
                P{data.total.proteina}g C{data.total.carboidratos}g G
                ...
                <div className="grid grid-cols-2 gap-2">
                  {commonFoods.map((food) => (
                    <button
                      key={food.id}
                      onClick={() =>
                        addItem(selectedMealKey as MealBlock["key"], food.id)
                      }
                ...
                <div className="max-h-60 overflow-y-auto">
                  {resultados.map((alimento) => (
                    <button
                      ...
                      onClick={() => {
                        addItem(
                          selectedMealKey as MealBlock["key"],
                          alimento.id,
                        );
                        setBusca("");
                      }}
                    ...
                    >
                      ...
                      <div className="flex-1 text-left">
                        ...
                        <div className="text-xs text-gray-500">
                          ...
                          {alimento.carboidratos}g G{alimento.gordura}g
                        </div>
                      </div>
                      <svg
                  ...
                  {resultados.length === 0 && (
                    <div className="p-4 text-center text-gray-500">
                      ...
                      <div className="text-xs mt-1">
                        Tente outros termos como "arroz", "frango", etc.
                      </div>
                    </div>
                  )}
                </div>
                    ...
                    <div className="border-t border-gray-200 p-4 space-y-3">
                              ...
                              <div className="border-t pt-2">
                                ...
                                {item.alternativas &&
                                  item.alternativas.map((alt, idx) => {
                                    ...
                                    return (
                                      <div
                                        key={`${alt.alimentoId}-${idx}`}
                                        className="flex items-center justify-between bg-white border border-amber-200 rounded px-3 py-2 mb-1"
                                      >
                                        <div className="flex items-center gap-2">
                                  ...
                                  <div className="flex gap-2 overflow-x-auto pb-2">
                                    <div className="mt-2 space-y-2">
                                      <input
                                        ...
                                        value={altSearch[item.id] || ""}
                                        onChange={(e) =>
                                          handleAltSearchChange(
                                            item.id,
                                            e.target.value,
                                          )
                                        }
                                      ...
                                      {altSearch[item.id] && (
                                        <div className="max-h-32 overflow-y-auto border rounded bg-white">
                                          {buscarAlimentos(altSearch[item.id])
                                            .slice(0, 10)
                                            .map((food) => (
                                              <button
                                                key={food.id}
                                                ...
                                                className="flex items-center gap-2 w-full px-2 py-1 hover:bg-amber-50 text-xs border-b last:border-b-0"
                                              >
                      ...
                      {meal.itens.length === 0 && (
                        <div className="text-center py-6 text-gray-500">
                          ...
                          <div className="text-xs mt-1">
                            Vá para a aba "Buscar Alimentos" para adicionar
                          </div>
                        </div>
                      )}
                    </div>
